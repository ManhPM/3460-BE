...
namespace App\Api\AdminV1\Services\Order;
...
class OrderService
{
    ...
    private function restoreProduct($order): void
    {
        ...
        foreach ($order->details as $detail) {
            ...
            if ($detail->product->is_flash_sale) {
                $flashSaleDetail = $detail->product->is_flash_sale->details()->firstWhere('product_id', $detail->product_id);
            ...
            if ($order && $order->status != OrderStatus::Cancelled && $order->status != OrderStatus::Completed) {
                ...
                if (($order->payment_method == \App\Enums\Payment\PaymentMethod::Wallet) && $order->user) {
                    ...
                    \App\Models\WalletTransaction::create([
                        ...
                        'order_id' => $order->id,
                        'note' => 'Hoàn tiền vào ví do huỷ đơn (admin API)',
                    ]);
                }
            } else {
                DB::rollBack();
    ...
    {
        $products = $this->productRepository->getByIdsAndOrderByIds(
    ...
    private function handleOrderCompleted($order, $pointsChange, $pointsChangeMembership): void
    {
        ...
        if ($pointsChange > 0) {
            $pointsMessage = __('order.points_received_message', ['points' => number_format($pointsChange, 0, ',', '.'), 'order_code' => $order->code]);
            $this->sendNotification(
                $user->id,
                __("order.point_earned_success"),
                $pointsMessage,
                $user->device_token
            );
        }
        ...
        foreach ($affiliateEarnings as $affiliateCode => $totalEarning) {
            ...
            if ($affiliateUser) {
                if ($totalEarning > 0) {
                    ...
                    $affiliateMessage = "Bạn đã nhận được " . number_format($totalEarning, 0, ',', '.') . " " . config('custom.currency') . " hoa hồng từ đơn hàng #{$order->code}";
                    $this->sendNotification(
                        $affiliateUser->id,
            ...
            foreach ($order->details as $detail) {
                if ($detail->product_variation_id) {
                    ...
                    if (!$inventory || $inventory->qty < $detail->qty) {
                        DB::rollBack();
                        throw new \Exception(__('order.insufficient_product', ['product_name' => $detail->productVariation->product->name ?? '']));
                    }
                    ...
                    if (!$inventory || $inventory->qty < $detail->qty) {
                        DB::rollBack();
                        throw new \Exception(__('order.insufficient_product', ['product_name' => $detail->product->name ?? '']));
                    }
                ...
                if ($detail->product->is_flash_sale) {
                    ...
                    if ($detail->productVariation) {
                        if ($detail->unit_price == $detail->productVariation->flashsale_price) {
                            DB::rollBack();
                            throw new \Exception(__('order.flash_sale_expired'));
                        }
                    } else {
    ...
    public function calculateTotal(Request $request)
    {
        ...
        foreach ($data['product_id'] as $key => $value) {
            if ($data['product_variation_id'][$key] == 0 || !$data['product_variation_id'][$key]) {
